...
declare type EventFor<T extends string, D = {}> = Expand<{
    type: T;
} & D>;
/**
 ...
 */
export declare type EventType = Event["type"];
/**
 ...
 */
export declare type EventOfType<E extends EventType = EventType> = Expand<Extract<Event, Event & EventFor<E>>>;
/**
...
declare type IfEquals<X, Y, A, B = never> = (<T>() => T extends X ? 1 : 2) extends <T>() => T extends Y ? 1 : 2 ? A : B;
declare type Expand<T> = T extends infer O ? {
    [K in keyof O]: O[K];
} : never;
/** @internal */
export declare type ReadableKeys<T> = {
...
export declare type Options<T, E = {}> = Expand<Partial<E & Pick<T, WritableKeys<T>>>>;
export {};
